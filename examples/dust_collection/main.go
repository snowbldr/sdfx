...
func main() {
	...
	render.ToSTL(s, "fdd_fvh25.stl", render.NewMarchingCubesOctreeParallel(150))

	s, err = vacuumToPipeMM("sch40:2")
	if err != nil {
		log.Fatalf("error: %s", err)
	}
	render.ToSTL(s, "mvh25_mpvc.stl", render.NewMarchingCubesOctreeParallel(150))

	s, err = dustDeputyToPipeFM("sch40:2")
	if err != nil {
		log.Fatalf("error: %s", err)
	}
	render.ToSTL(s, "fdd_mpvc.stl", render.NewMarchingCubesOctreeParallel(150))
}
